...
def do_slotting(
    ...
    """
    Does a bunch of string replacement to make the templates look like normal sentences based on whatever
    ...
    Args:
        ...
        lang: language code (default: "en"). English-specific cleanup (a/an, a couple of hacks) is only applied when lang=="en".
    ...
    for a in range(len(frame_cols)):
        ...
        if isinstance(this_txt, str):
            ...
            if lang == "en":
                if "lesbian man" in this_txt:  # easiest way to fix an issue in sexual_orientation
                    this_txt = this_txt.replace("lesbian man", "lesbian woman")
                if "Latino woman" in this_txt:  # easiest way to fix this issue too
